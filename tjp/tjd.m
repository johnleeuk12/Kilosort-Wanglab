...
%                         pool.templates.B{i} = x.s_unit.templates.data;
                ...
                d_template{seg_ls} = zeros(length(A),length(B));
                ...
                for i = 1:obj.N
                    for j =  1:obj.N
                        if obj.SU_good.A(i)*obj.SU_good.B(j) ~= 0 %if SU on both segments, calculate distance
                            for ch = 1:64
                                temp = sqrt(sum((pool.templates.A{i}(ch,:)- pool.templates.B{j}(ch,:)).^2));
                                d_template{seg_ls}(i,j) = d_template{seg_ls}(i,j) + temp;
                            end
                        else
                            d_template{seg_ls}(i,j) = NaN;
                        end
                    end
                end
            ...
            for u = 1:obj.N
                ...
                %                 grp3 = test(idx == 7);
                idx2 = idx(idx ~=u);
                Y1 = prctile(grp2,5); %2.5 percentile of group 2
                Y2 = prctile(grp2,8);
